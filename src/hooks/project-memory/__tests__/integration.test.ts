...
describe("Project Memory Integration", () => {
  ...
  describe("End-to-end SessionStart flow", () => {
    ...
    it("should not inject if project has no useful info", async () => {
      // Create a minimal project root marker (empty .git) but no useful config files
      // This ensures findProjectRoot finds THIS directory, not a parent
      await fs.mkdir(path.join(tempDir, ".git"));

      const sessionId = "test-session-4";
      const registered = await registerProjectMemoryContext(sessionId, tempDir);

      // Should return false because there are no languages, frameworks, or build commands detected
