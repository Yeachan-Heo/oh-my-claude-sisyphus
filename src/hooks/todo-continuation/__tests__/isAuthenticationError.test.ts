...
import type { StopContext } from "../index.js";
...
describe("isAuthenticationError", () => {
  ...
  it("should detect authentication_error", () => {
    expect(isAuthenticationError({ stop_reason: "authentication_error" })).toBe(
      true,
    );
  });
  ...
  it("should detect authentication_failed", () => {
    expect(
      isAuthenticationError({ stop_reason: "authentication_failed" }),
    ).toBe(true);
  });
  ...
  it("should detect invalid_api_key", () => {
    expect(isAuthenticationError({ stop_reason: "invalid_api_key" })).toBe(
      true,
    );
  });

  it("should detect api_key_expired", () => {
    expect(isAuthenticationError({ stop_reason: "api_key_expired" })).toBe(
      true,
    );
  });
  ...
  it("should detect permission_denied", () => {
    expect(isAuthenticationError({ stop_reason: "permission_denied" })).toBe(
      true,
    );
  });
  ...
  it("should detect credentials_expired", () => {
    expect(isAuthenticationError({ stop_reason: "credentials_expired" })).toBe(
      true,
    );
  });
  ...
  it("should detect compound authentication_error patterns", () => {
    expect(
      isAuthenticationError({ stop_reason: "api_authentication_error" }),
    ).toBe(true);
  });
  ...
  it("should detect auth errors in end_turn_reason field", () => {
    expect(
      isAuthenticationError({ end_turn_reason: "authentication_error" }),
    ).toBe(true);
    expect(isAuthenticationError({ end_turn_reason: "unauthorized" })).toBe(
      true,
    );
  });
