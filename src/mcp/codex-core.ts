...
export const CODEX_DEFAULT_MODEL =
  process.env.OMC_CODEX_DEFAULT_MODEL || "gpt-5.2";
export const CODEX_TIMEOUT = Math.min(
  Math.max(
    5000,
    parseInt(process.env.OMC_CODEX_TIMEOUT || "3600000", 10) || 3600000,
  ),
  3600000,
);

// Codex is best for analytical/planning tasks
export const CODEX_VALID_ROLES = [
  "architect",
  "planner",
  "critic",
  "analyst",
...
export async function handleAskCodex(args: {
  ...
  if (
    !agent_role ||
    !(CODEX_VALID_ROLES as readonly string[]).includes(agent_role)
  ) {
    return {
      content: [
        {
