...
// Check if prompt is an internal CLI command that should be skipped
// Fixes infinite loop when rate limit is hit during ultrawork mode (#29)
function isInternalCommand(prompt) {
  // Skip rate-limit commands, internal commands, and bare slash commands
  return /^\/?(rate-limit|internal-|___|---)|^\/[a-z-]+$/i.test(prompt.trim());
}
...
async function main() {
  try {
    ...
    // Skip internal CLI commands (rate-limit-options, etc.)
    if (isInternalCommand(prompt)) {
      console.log(JSON.stringify({ continue: true }));
      return;
    }
