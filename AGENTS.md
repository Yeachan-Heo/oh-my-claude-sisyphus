...
- **32 hooks** for event-driven execution modes and enhancements
...
│  │ (38 skills) │ (32 agents) │(LSP/AST/REPL)│ (32 hooks)  │  │
│  └─────────────┴─────────────┴─────────────┴─────────────┘  │
│  ┌─────────────────────────────────────────────────────────┐│
│  │              Features Layer                             ││
...
Key skills: `autopilot`, `ultrawork`, `ralph`, `ultrapilot`, `plan`, `ralplan`, `deepsearch`, `deepinit`, `frontend-ui-ux`, `git-master`, `tdd`, `security-review`, `code-review`, `research`, `analyze`, `swarm`, `pipeline`, `ecomode`, `cancel`, `learner`, `note`, `hud`, `doctor`, `omc-setup`, `mcp-setup`, `build-fix`, `ultraqa`, `refactor`
...
| Parameter   | Indexing                | Notes                                              |
| ----------- | ----------------------- | -------------------------------------------------- |
| `line`      | 1-indexed (user-facing) | Internally converted to 0-indexed for LSP protocol |
| `character` | 0-indexed               | Position within the line                           |

**For `lsp_goto_definition`**: Position cursor on the symbol NAME, not the start of line.

### AST-grep Pattern Syntax

| Meta-variable | Purpose                   | Example                |
| ------------- | ------------------------- | ---------------------- |
| `$NAME`       | Single AST node           | `function $NAME() {}`  |
| `$$$`         | Multiple nodes (variadic) | `console.log($$$ARGS)` |

**Important**: Patterns must be complete AST nodes. Escape `$$$` in markdown with code blocks.

### Performance Guidelines

| Practice       | Recommendation                                                          |
| -------------- | ----------------------------------------------------------------------- |
| Regex patterns | Pre-compile at module level, not inside functions                       |
| Type safety    | Use `Record<string, unknown>` instead of `Record<string, any>`          |
| JSON writes    | Avoid `JSON.stringify(x, null, 2)` formatting in production file writes |
| Global regex   | Reset `lastIndex = 0` before `.test()` calls on global regexes          |
