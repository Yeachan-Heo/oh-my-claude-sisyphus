...
## CLI Workers (Hybrid Roles)

The team skill supports **hybrid execution** combining Claude agent teammates with tmux CLI workers. Both types can make code changes -- they differ in capabilities and interaction model.
...
| `cli_worker` | Tmux CLI session | Full filesystem access in working_directory. Runs autonomously via tmux pane. Best for code review, security analysis, refactoring, architecture. |
...
Task decomposition:
#1 [cli_worker] Security review of current auth code -> output to .omc/research/auth-security.md
#2 [cli_worker] Refactor auth/login.ts and auth/session.ts (uses #1 findings)
...
#5 [cli_worker] Final code review of all changes
```

The lead runs #1 (CLI security analysis), then #2 and #3 in parallel (CLI refactors backend, designer agent redesigns frontend), then #4 (Claude teammate handles test iteration), then #5 (final CLI review).
