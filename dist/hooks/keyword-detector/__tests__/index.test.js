...
import { removeCodeBlocks, sanitizeForKeywordDetection, extractPromptText, detectKeywordsWithType, hasKeyword, getPrimaryKeyword, getAllKeywords, getAllKeywordsWithSizeCheck, } from '../index.js';
// Mock isTeamEnabled
    ...
    describe('getAllKeywordsWithSizeCheck', () => {
        describe('small task suppression', () => {
            ...
            it('does NOT suppress ralph on large task (refactor)', () => {
                const result = getAllKeywordsWithSizeCheck('ralph refactor the entire authentication module to support OAuth2 and update all related tests');
        ...
        describe('custom thresholds', () => {
            it('uses custom smallWordLimit', () => {
                ...
                const result = getAllKeywordsWithSizeCheck('ralph fix the bug in the handler function now please', { smallWordLimit: 3, largeWordLimit: 200 });
                expect(result.keywords).toContain('ralph');
                expect(result.suppressedKeywords).not.toContain('ralph');
            });
        });
