    ...
    for (const line of lines) {
        try {
            ...
            if (event.type === 'error' || event.type === 'turn.failed') {
                ...
                if (/model_not_found|model is not supported|does not exist|do not have access/i.test(msg)) {
...
overrides, reasoningEffort) {
    ...
    const effectiveModel = model || fallbackChain?.[0] || CODEX_DEFAULT_MODEL;
...
export function executeCodexBackground(fullPrompt, modelInput, jobMeta, workingDirectory, reasoningEffort) {
    try {
        ...
        const effectiveModel = modelInput || jobMeta.model || CODEX_DEFAULT_MODEL;
