...
export function executeGeminiBackground(fullPrompt, modelInput, jobMeta, workingDirectory) {
    try {
        ...
        // Use the job metadata's resolved model (from policy) as fallback, not the hardcoded default
        const effectiveModel = modelInput || jobMeta.model || GEMINI_DEFAULT_MODEL;
...
${resolvedPrompt}`;
    ...
    for (const tryModel of fallbackChain) {
        try {
            ...
            if (!/model error|model.?not.?found|model is not supported|does not exist|do not have access|429|rate.?limit|too many requests|quota.?exceeded|resource.?exhausted/i.test(errMsg)) {
